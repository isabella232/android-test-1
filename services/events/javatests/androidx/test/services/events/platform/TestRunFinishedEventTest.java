...
public class TestRunFinishedEventTest {
  ...
  public void testRunFinishedEvent_created_by_createFromParcel() {
    ...
    TestStatus runStatus = new TestStatus(Status.ABORTED);

    TestRunFinishedEvent testRunFinishedEvent =
        new TestRunFinishedEvent(testRun, runStatus, timestamp);
    ...
    assertThat(testPlatformEventFromParcel).isInstanceOf(TestRunFinishedEvent.class);

    TestRunFinishedEvent result = (TestRunFinishedEvent) testPlatformEventFromParcel;
    ...
    assertThat(result.runStatus.status).isEqualTo(runStatus.status);
